...
const TABS: TabData[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { label: "Session ready to share", done: false, active: true },
    ],
    card: {
  ...
  },
  {
    ...
    steps: [
      { label: "Recording typing patterns", done: true },
      { label: "Flagging pause points", done: true },
      { label: "Running ai code review", done: true },
      { label: "Scoring candidate performance", done: false, active: true },
      { label: "Publishing session report", done: false },
    ],
    card: {
...
const ArrowToGenerator = ({ position }: { position: DescPosition }) => {
  ...
  const rotations: Record<DescPosition, string> = {
    "right-bottom": "scaleY(-1)", // arrow points ↙ toward gen (top-left)
  ...
  const styles: Record<DescPosition, React.CSSProperties> = {
    "right-bottom": { left: "-2.8rem", top: "-2.1rem" },
    "left-top": { right: "-4.5rem", bottom: "10rem" },
    "left-bottom": { right: "-3rem", top: "10rem" },
    "right-top": { left: "-4.5rem", bottom: "10rem" },
  };
  ...
  return (
    ...
    >
      <path
        d='M130 10 C 130 80, 80 100, 20 100'
        stroke='#00FF41'
        strokeWidth='1.5'
        strokeLinecap='round'
        strokeDasharray='4 4'
...
export const GeneratorSection: React.FC = () => {
  ...
  return (
    <section className='relative w-full px-4 sm:px-6 pt-16 sm:pt-20 md:pt-24 pb-20 sm:pb-28 md:pb-32 flex flex-col items-center bg-transparent z-10'>
      ...
      <LayoutGroup>
        <div
          className='w-full max-w-[1050px] relative z-20 hidden lg:flex items-start gap-10'
          style={{ flexDirection: isDescRight ? "row" : "row-reverse" }}
        >
          ...
          >
            ...
            <div className='flex items-center justify-between px-6 pt-6'>
              ...
              <div className='flex items-center gap-3'>
                <span className='text-[#00FF41] text-[10px] font-bold font-mono'>
                  ↑ 94% match
                </span>
                <div className='w-5 h-5 rounded-full bg-white/10'></div>
                <div className='px-3 py-1 bg-white/5 rounded-full text-[10px] text-white/30'>
                  Publish
                </div>
              </div>
            </div>
            ...
            <AnimatePresence mode='wait'>
              ...
              >
                <h3 className='text-2xl font-medium text-white mb-0.5'>
                  {current.title}
                </h3>
                <h3 className='text-2xl font-medium text-[#00FF41] mb-6'>
                  {current.titleAccent}
                </h3>
                <div className='flex flex-col gap-3 w-full max-w-[240px]'>
                  {current.steps.map((step, i) => (
                    <div
                      key={i}
                      className={`flex items-center gap-3 ${!step.done && !step.active ? "opacity-25" : ""}`}
                    >
                      {step.done ? (
                        <CheckIcon />
                      ) : step.active ? (
          ...
          >
            <motion.div
              layout='position'
              transition={spring}
              className='bg-[#131313] rounded-3xl p-10 shadow-[0_20px_60px_rgba(0,0,0,0.5)] border border-white/[0.04] min-h-[280px] antialiased'
              style={{ 
                rotate: current.cardRotate, 
      ...
      <div className='w-full max-w-[600px] relative z-20 flex flex-col items-center gap-8 lg:hidden'>
        <div className='w-full aspect-[6/5] bg-[#0d0d0d] rounded-2xl sm:rounded-3xl border border-white/[0.06] relative overflow-hidden shadow-2xl'>
          ...
          <div className='flex items-center justify-between px-4 sm:px-6 pt-4 sm:pt-6'>
            ...
            <div className='flex items-center gap-2 sm:gap-3'>
              <span className='text-[#00FF41] text-[9px] sm:text-[10px] font-bold font-mono'>
                + 330
              </span>
          ...
          <AnimatePresence mode='wait'>
            <motion.div
              ...
              className='absolute inset-x-0 top-1/2 -translate-y-1/2 flex flex-col items-center px-4'
            >
              <h3 className='text-lg sm:text-xl font-medium text-white mb-0.5'>
                {current.title}
              </h3>
              <h3 className='text-lg sm:text-xl font-medium text-[#00FF41] mb-5'>
                {current.titleAccent}
              </h3>
              <div className='flex flex-col gap-2.5 w-full max-w-[240px]'>
                {current.steps.map((step, i) => (
                  <div
                    key={i}
                    className={`flex items-center gap-2.5 ${!step.done && !step.active ? "opacity-25" : ""}`}
                  >
                    {step.done ? (
                      <CheckIcon />
                    ) : step.active ? (
        ...
        <AnimatePresence mode='wait'>
          ...
          >
            <div className='bg-[#131313] rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-[0_20px_60px_rgba(0,0,0,0.5)] border border-white/[0.04] min-h-[220px] antialiased transform-gpu'>
